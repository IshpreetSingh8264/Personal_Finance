            ...
            <header ref={navbarRef} className="fixed top-0 left-0 w-full bg-[#121212] border-b border-[#292929] z-50">
                <nav className="max-w-[85rem] mx-auto px-6 flex items-center justify-between py-3">
                    ...
                    <div className="hidden sm:flex items-center gap-6">
                        {["Home", "Transaction", "Contact Us", "Work", "Blog"].map((item, index) => (
                            <Link key={index} to={item === "Contact Us" ? "/contactus" : item === "Transaction" ? "/transaction" : "/"} className="relative text-[#B0B0B0] hover:text-[#4CAF50] transition-all duration-300 font-medium group">
                                {item}
                    ...
                    {isOpen && (
                        ...
                        >
                            <div className="mobile-menu">
                                {["Home", "Transaction", "Contact Us", "Work", "Blog"].map((item, index) => (
                                    <Link key={index} to={item === "Contact Us" ? "/contactus" : item === "Transaction" ? "/transaction" : "/"} className="text-[#B0B0B0] hover:text-[#4CAF50] transition-all duration-300 font-medium">
                                        {item}
                                    </Link>
                                ))}
